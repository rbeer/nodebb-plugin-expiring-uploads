...
class ExpiringFile {
   ...
   * > '70042d29'
   * expFile.uploadMeta.expTstamp;
   * > 1458436960496
   * expFile.fileName;
   * > '1458436960496-stuff.txt'
   *
   * expFile.url;
   * > 'https://localhost:4567/uploads/70042d29/1458436960496/stuff.txt'
   */
  get url() {
    let thisMeta = this._uploadMeta;
    let urlName = this._fileName.replace(/^\d*-/, '');
    return nconf.get('url') + nconf.get('upload_url') +
           `${thisMeta.hash}/${thisMeta.expTstamp}/${urlName}`;
  }
}
