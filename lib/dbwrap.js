...
class DBWrap {
  ...
  static saveUpload(uploadData, cb) {
    let id = Settings().nextId;
    let file = new ExpiringFile(id, uploadData);
    async.waterfall([
      function(next) {
        DB.sortedSetAdd('expiring-uploads:ids', uploadData.expTstamp, id, (err) => next(err, id));
      },
      function(id, next) {
  ...
  /**
   * Gets ID of a file by its timestamp
   * @param  {number}   tstamp - Score in expiring-uploads:ids
   * @param  {Function} cb
   */
